  ...
  (letrec ([type-name-python (get-python-type-name (member-def-type memb) module)]
    ...
    (string-append
      ...
      (string-append
          ...
          [(callable-def? origin-type)
             (string-append
                "void * _pyfunc_value = NULL;\n"
                "void * _pyclosure_value = NULL;\n"
                (check-arg-block origin-type "value" module "-1" "" )
                (format "Py_XDECREF(self->data.~a_closure);\n" name )
                (format "self->data.~a_func = _pyclosure_value;\n" name)
                (format "self->data.~a_closure = _pyclosure_value;\n" name)
                (format "Py_XINCREF(self->data.~a_closure);\n" name )
                "return 0;\n}\n")]
...
(define (check-arg-block arg-type arg-name module ret-val [prefix "_pyarg_"] [prefix-callable "_pyargdata_"])   
  (cond 
    ...
    [(callable-def? arg-type) 
      (callable-check-block arg-type prefix arg-name  (format "_py_is_python_~a" arg-name) (get-python-type-name arg-type module) (get-c-type-name arg-type module) module ret-val)]
...
(define (callable-check-block type prefix arg-name py_is_callable py-type-name c-name module ret-value) 
  (string-append
    (format "    if (!PyObject_TypeCheck(~a~a,\n" prefix arg-name)
    (format "    _get~a())) {\n" py-type-name)
    (format "     PyObject * _pytempobj_ = (PyObject * )~a~a;\n" prefix arg-name)
    ...
    "}else{\n"
    (format "    mir_callable * _temp_callable_ = _get_data~a(~a~a);\n" py-type-name prefix arg-name   )
